...
export interface RequestPermissionOptions {
  ...
  sessionLabel?: string;
  /**
   * Timeout in milliseconds for the permission prompt. When the timeout expires,
   * the request is automatically denied with `timeoutDenialReason`.
   * Set to 0 or omit to disable timeout (default behavior).
   * Ported from MasuRii/pi-permission-system.
   */
  timeoutMs?: number;
  /**
   * Denial reason to use when the prompt times out.
   * Only meaningful when `timeoutMs` is set and > 0.
   */
...
export async function requestPermissionDecisionFromUi(
  ...
  if (hasTimeout) {
    const TIMEOUT_SENTINEL = Symbol("forwarded-permission-timeout");
    const timeoutPromise: Promise<symbol> = new Promise((resolve) => {
      setTimeout(() => resolve(TIMEOUT_SENTINEL), options!.timeoutMs!);
    });
    ...
    // Timeout expired — auto-deny with configured reason
    if (result === TIMEOUT_SENTINEL) {
      return {
        approved: false,
        state: "denied_with_reason",
        denialReason:
          options!.timeoutDenialReason || "Permission request timed out.",
      };
    }
