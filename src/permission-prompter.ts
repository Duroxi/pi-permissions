...
import { shouldAutoApproveForAllowEdits } from "./allow-edits-mode";
...
export interface PromptPermissionDetails {
  ...
  sessionLabel?: string;
  /**
   * Whether the path being accessed is outside the current working directory.
   * Used by allowEdits mode to reject auto-approval for external paths.
...
export class PermissionPrompter implements PermissionPrompterApi {
  ...
  async prompt(
    ...
    // AllowEdits mode: auto-approve write/edit surfaces for in-CWD paths.
    const surface =
  ...
  private writeReviewEntry(
    ...
    this.deps.logger.review(event, {
      ...
      isExternalPath: details.isExternalPath ?? null,
    });
  }
}
