...
import {
  ...
  normalizeOptionalNullablePositiveInt,
...
export interface UnifiedPermissionConfig {
  ...
  yoloMode?: boolean;
  /** Permission mode: "default" | "allowEdits" | "yolo". Overrides yoloMode when set. */
  mode?: string;
  toolInputPreviewMaxLength?: number;
  toolTextSummaryMaxLength?: number;
  piInfrastructureReadPaths?: string[];
  /** Timeout in seconds for forwarded permission prompts. */
...
export function normalizeUnifiedConfig(raw: unknown): {
  ...
  const forwardedPromptTimeoutSeconds = normalizeOptionalNullablePositiveInt(
    record.forwardedPromptTimeoutSeconds,
  );
  if (forwardedPromptTimeoutSeconds !== undefined)
    config.forwardedPromptTimeoutSeconds = forwardedPromptTimeoutSeconds;
...
export function mergeUnifiedConfigs(
  ...
  // Nullable number scalars: override replaces base when defined
  {
    const value = override.forwardedPromptTimeoutSeconds ?? base.forwardedPromptTimeoutSeconds;
    if (value !== undefined) {
      merged.forwardedPromptTimeoutSeconds = value;
    }
  }
