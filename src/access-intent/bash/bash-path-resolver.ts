...
/** Default promotion predicate: promotes nothing . */
...
export interface ResolvedBashPaths {
 /** Deduplicated paths resolving outside the working directory . */
 readonly externalPaths: readonly AccessPath[];
 /** Every path-rule token paired with its cd-aware policy values . */
 ...
 * The resolver never sees the rules themselves — the predicate is
...
export class BashPathResolver {
 ...
 private walkForCandidates(
  ...
  switch (node.type) {
   ...
   case "pipeline":
    ...
    // command and every downstream stage as non-folding subshells.
  ...
  * and collects tokens against the folded base without folding.
  */
 ...
 private projectExternalPaths(
  ...
  for (const { token, base } of candidates) {
   ...
   // config patterns match the path as the user typed it.
   const canonical = accessPath.boundaryValue();
  ...
  * classifier (`classifyPromotedRuleCandidate`) for a bare token the
  ...
  * spurious absolute rule can match.
  */
