...
export class ToolCallGatePipeline {
  ...
  async evaluate(
    ...
    const gateProducers: Array<() => GateResult | Promise<GateResult>> = [
      ...
      () => {
        const { toolCheck, accessPath, filePath } = this.resolvePerToolCheck(
          tcc,
          bashProgram,
          command,
          normalizer,
        );
        const isExternalPathForGate = filePath
          ? normalizer.isOutsideWorkingDirectory(filePath)
          : undefined;
  ...
  ): { toolCheck: PermissionCheckResult; accessPath?: AccessPath; filePath?: string } {
