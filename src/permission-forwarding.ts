...
export type ForwardedPermissionRequest = {
  ...
  value?: string | null;
  /**
   * Cryptographic nonce embedded in the request, must be echoed back in the
   * response to prevent response forgery attacks on the file-based IPC channel.
   * Optional for version-skew tolerance with older children.
...
export type ForwardedPermissionResponse = {
  ...
  respondedAt: number;
  /**
   * Cryptographic nonce echoed from the request. The child verifies this
   * matches the original request nonce before accepting the response.
   * Optional for version-skew tolerance with older parents.
   * Ported from MasuRii/pi-permission-system.
   */
  responseNonce?: string;
};
...
/**
 * Generate a cryptographic nonce for permission forwarding response binding.
 * 32 bytes of cryptographic randomness encoded as base64url.
 * Ported from MasuRii/pi-permission-system.
 */
export function createPermissionForwardingNonce(): string {
...
/**
 * Verify that a forwarded permission response is cryptographically bound to
 * its original request. Checks:
 * 1. The response nonce matches the request nonce (timing-safe)
 * 2. The responder session ID matches the expected target session
 *
 * Returns true if the response is valid; false if it should be rejected.
 * Ported from MasuRii/pi-permission-system.
 */
export function isForwardedPermissionResponseBoundToRequest(
  request: Pick<ForwardedPermissionRequest, "responseNonce" | "targetSessionId">,
  response: Pick<ForwardedPermissionResponse, "responseNonce" | "responderSessionId">,
): boolean {
  // If the request had no nonce (version-skew with older child), skip nonce
  // verification but still validate the responder session.
  if (request.responseNonce) {
    if (!response.responseNonce) {
      return false;
    }
    if (!safeEqualString(request.responseNonce, response.responseNonce)) {
      return false;
    }
  }

  // Verify the responder is the expected target session.
  const normalizedTarget = normalizePermissionForwardingSessionId(
    request.targetSessionId,
  );
  const normalizedResponder = normalizePermissionForwardingSessionId(
    response.responderSessionId,
  );
  if (!normalizedTarget || !normalizedResponder) {
    return false;
  }
  return safeEqualString(normalizedTarget, normalizedResponder);
}
