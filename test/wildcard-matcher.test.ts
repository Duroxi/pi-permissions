...
describe("ReDoS protection (MAX_WILDCARD_PATTERN_LENGTH)", () => {
  test("MAX_WILDCARD_PATTERN_LENGTH is 500", () => {
    expect(MAX_WILDCARD_PATTERN_LENGTH).toBe(500);
  });

  test("pattern within length limit compiles normally", () => {
    const pattern = "a".repeat(499);
    const compiled = compileWildcardPattern(pattern, "allow");
    expect(compiled.regex.test(pattern)).toBe(true);
  });

  test("pattern at exact length limit compiles normally", () => {
    const pattern = "a".repeat(500);
    const compiled = compileWildcardPattern(pattern, "allow");
    expect(compiled.regex.test(pattern)).toBe(true);
  });

  test("pattern exceeding length limit returns never-match regex", () => {
    const pattern = "a".repeat(501);
    const compiled = compileWildcardPattern(pattern, "allow");
    // Never-match pattern never matches any input, including empty string
    expect(compiled.regex.test(pattern)).toBe(false);
    expect(compiled.regex.test("anything")).toBe(false);
    expect(compiled.regex.test("a")).toBe(false);
  });

  test("long wildcard pattern with * exceeding limit never matches", () => {
    const pattern = "*".repeat(501);
    const compiled = compileWildcardPattern(pattern, "deny");
    expect(compiled.regex.test("any value")).toBe(false);
  });

  test("long pattern preserves original pattern and state", () => {
    const pattern = "x".repeat(600);
    const compiled = compileWildcardPattern(pattern, "ask");
    expect(compiled.pattern).toBe(pattern);
    expect(compiled.state).toBe("ask");
  });

  test("wildcardMatch rejects patterns exceeding length limit", () => {
    const longPattern = "a".repeat(501);
    expect(wildcardMatch(longPattern, "a".repeat(501))).toBe(false);
  });

  test("findCompiledWildcardMatch skips over-length patterns", () => {
    const patterns = compileWildcardPatternEntries([
      ["a".repeat(501), "deny"],
      ["normal", "allow"],
    ]);
    const result = findCompiledWildcardMatch(patterns, "a".repeat(501));
    // The over-length pattern should not match
    expect(result).toBeNull();
    // But the normal pattern should still work
    const normalResult = findCompiledWildcardMatch(patterns, "normal");
    expect(normalResult).not.toBeNull();
    expect(normalResult?.state).toBe("allow");
  });

  test("tilde expansion cannot bypass length limit", () => {
    // A pattern under the raw limit that expands via ~ to exceed the limit.
    // ~ expands to "/home/testuser" which is 14 chars.
    // So "~/" (2 chars) + 498 chars = 500 total (at limit).
    // After expansion: "/home/testuser/" (15 chars) + 498 = 513 chars > 500.
    const rawPattern = "~/".concat("a".repeat(498));
    expect(rawPattern.length).toBe(500); // raw is AT the limit
    const compiled = compileWildcardPattern(rawPattern, "allow");
    // Expanded len = 15 + 498 = 513 > 500 → must be rejected
    expect(compiled.regex.test(rawPattern)).toBe(false);
    expect(compiled.state).toBe("allow");
  });
});
